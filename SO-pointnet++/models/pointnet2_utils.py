...
from TripleAttention import TripleAttention
"""
打印时间
"""
def timeit(tag, t):
...
class PointNetSetAbstractionMsgAttention(nn.Module):
    def __init__(self, npoint, radius_list, nsample_list, in_channel, mlp_list):
        ...
        self.mlp_attention0 = ShuffleAttention(mlp_list[0][0])
        ...
        self.mlp_attention1 = ShuffleAttention(mlp_list[1][0])
        self.mlp_conv11 = Conv(mlp_list[1][0],mlp_list[1][1],1)
        self.mlp_conv12 = Conv(mlp_list[1][1],mlp_list[1][2],1)

        self.mlp_conv20 = Conv(in_channel + 3, mlp_list[2][0], 1)
        self.mlp_attention2 = ShuffleAttention(mlp_list[2][0])
        self.mlp_conv21 = Conv(mlp_list[2][0], mlp_list[2][1], 1)
        self.mlp_conv22 = Conv(mlp_list[2][1], mlp_list[2][2], 1)
        ...
        for i, radius in enumerate(self.radius_list):
            ...
            if i==0:
               ...
               grouped_points = self.mlp_conv02(grouped_points)
                ...
                grouped_points = self.mlp_conv20(grouped_points)
                grouped_points = self.mlp_attention2(grouped_points)  # 注意力机制
                grouped_points = self.mlp_conv21(grouped_points)
                grouped_points = self.mlp_conv22(grouped_points)
...
class PointResnet(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        Return:
            ...
            new_points_concat: sample points feature data, [B, D, N]
        """
        xyz = xyz.permute(0, 2, 1)
        if points is not None:
            Respoints = points
